...
private:
	...
	cv::Mat copyLeft, copyRight, frametresh;
	...
	double wheelSteering, correction, diffAngle, finalAngle;
	int tolerance;
...
public:
	...
	ProcessC() {
		...
		{   //nie som si isty ci potrebujeme mat vsetky atributy nacitane
			...
			correction = boost::lexical_cast<double>(pt.get<string>("ProcessC.correction"));
			wheelSteering = 0;
                        finalAngle = 0;
	...
	void process(cv::Mat frameLeft, cv::Mat frameRight) {
		...
		cv::Mat frameHls, frameGauss, frameAdded, frameCanny, frameRoi;
