...
public:
	...
	void laneAssist(cv::Mat frameRoi, cv::Mat imageRoi) {
		...
		int yBase = (int)(imageRoi.size().height -50); // zakladna trojuholnika, 50 od spodu
		int yVertex = 20;

		cv::Mat leftPartImg, rightPartImg;
		leftPartImg = cutImage(frameRoi, 0, 0, frameRoi.size().width / 2,
			frameRoi.size().height); //lava cast obrazka
		rightPartImg = cutImage(frameRoi, frameRoi.size().width / 2, 0,
			frameRoi.size().width / 2, frameRoi.size().height); // prava cast obrazka

		leftPartPoints = findLines(leftPartImg); // vyhlada ciary pre lavy obrazok
		medianLeftPoints = computeMedian(leftPartPoints); // urobi priemer ciar
		rightPartPoints = findLines(rightPartImg);  // vyhlada ciary pre pravy obrazok
		medianRightPoints = computeMedian(rightPartPoints); // urobi priemer ciar
		...
		medianRightPoints = ascendPoints(medianRightPoints); // usporiadanie ciar
		medianLeftPoints = ascendPoints(medianLeftPoints);

		extendLeftPoints = extendPoints(imageRoi, medianLeftPoints); // predlzenie ciar
		extendRightPoints = extendPoints(imageRoi, medianRightPoints);

		drawMiddleLine(imageRoi, cv::Scalar(0, 0, 0)); // stredna ciara

		if (!extendLeftPoints.empty()) {
			line(imageRoi, extendLeftPoints[0], extendLeftPoints[1], leftLineColor, 1,
				8, 0);//vykreslenie tenkych ciar
		}
	...
	*/
	vector<double> calculateWheelAngle(cv::Mat image, vector<cv::Point> leftPoints,
		...
		vector<double> result;
		double coefficient, angle, delta;
...
*/
		...
		angle = (double)(atan((double)(xVS - xBS) / (yBS - yVS)) * 180 / PI);
		...
		if (angle > 0) { drawWheelAngle(image, angle, xBS, yBS, xVS, yVS, cv::Scalar(0, 0, 255)); }
		else { drawWheelAngle(image, angle, xBS, yBS, xVS, yVS, cv::Scalar(255, 0, 0)); }
