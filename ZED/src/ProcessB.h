...
public:
	...
	ProcessB(){
		...
		cv::resizeWindow("LaneDetect control", 400, 500);
		cv::moveWindow("LaneDetect control", 10, 0);
	}
	...
	void process(cv::Mat frameLeft, cv::Mat frameRight){
		//while (!GetAsyncKeyState(VK_ESCAPE))
			...
			cv::Mat birdViewCapture;
			cvtColor(frameLeft, frameLeft, cv::COLOR_BGR2GRAY);
			...
			cv::Mat undistorted = cv::Mat(cvSize(320, 240), CV_8UC1);
			...
			GaussianBlur(birdViewCapture, birdViewCapture, cv::Size(3, 3), 0, 0, cv::BORDER_DEFAULT);
			cv::Mat sobelx64f, abs_sobelx64f, sobel8u;
			...
			cv::Mat lSobel, rSobel;
			lSobel = abs_sobelx64f(cv::Rect(0, 0, 160, 240));
			rSobel = abs_sobelx64f(cv::Rect(160, 0, 160, 240));
			...
			for (size_t i = lines1.size() - 2; i < lines1.size(); i++)
			{
				...
				pt.y = -1000;
				line(lineSobel, pt1, pt2, cv::Scalar(0, 255, 255), 1, CV_AA);
			}
			...
			for (size_t i = lines2.size() - 2; i < lines2.size(); i++)
			{
				...
				pt4.x = pt2.x + 160;
				pt4.y = pt2.y;
				line(lineSobel, pt3, pt4, cv::Scalar(0, 255, 255), 1, CV_AA);
			}
